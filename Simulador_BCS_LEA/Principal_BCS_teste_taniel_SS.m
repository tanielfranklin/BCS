...
%  freq = ones(nsim)*40;                   % [Hz] frequencia de operacao
%  val_pneumatica = ones(nsim)*20;                    % [%]  v�lvula pneum�tica
...
temperatura_fundo = Cexp.temperatura_fundo(1:nsim);    
    
%pressao_reser =  ones(nsim)*2.1788*1e5;                  % [Pa] pressao de reservatorio do LEA
%pressao_manifold = ones(nsim)*1.3394*1e04;           % [Pa] Press�o de manifold choke
...
xo=[pressao_fundo(end);pressao_choke(end);vazao_choke(end)]
