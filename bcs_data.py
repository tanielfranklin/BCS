...
yss = np.array([[6000142.88550200], [592.126490003812], [89533.9403916785], [35.8135761566714], [0.00995042241351779],
                [0.00995042241351780]]);
...
tg = 2;  # % MV target
# % Parametros
ts = 0.1;  # % tempo de amostragem (ts = 1 não converge)

# % Restrição
umin = np.array([35, 0]);
np.transpose(umin);  # lower bounds of inputs
umax = np.array([65, 100]);
np.transpose(umax);  # upper bounds of inputs
dumax = np.array([0.5, dzc_max]);
np.transpose(dumax);  # maximum variation of input moves (ts*[0.5 dzc_max]')
...
Modelo_Predicao = Function('Modelo_Predicao', [BCS['x'], BCS['u']], [res['xf'], Eq_medicao(res['xf'], BCS['u'])],
                           ['xk_1', 'uk_1'], ['xk', 'yk']);
...
for k in range(1, nsim):
    ...
    xpk[-2] = 60
    # xss = np.array([
    # [8311024.82175957], pbh
    # [2990109.06207437], pwh
    # [0.00995042241351780], dqdt
    # [50], fq
    # [50]]); zc
...
fig1 = plt.figure()
label = [r'$p_{in}(bar)$', 'H(m)'];
xi = (np.arange(0, nsim));
for iy in range(0, npv):
    ax = fig1.add_subplot(2, 1, iy + 1)
