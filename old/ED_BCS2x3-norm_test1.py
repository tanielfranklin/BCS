...
def ED_BCS(t,x):
    ...
    qr = PI * (pr - pbh);
    qch = Cc * (zc / 100) * tf.sign((pwh - pm)) * tf.sqrt(tf.abs(pwh - pm));
    ...
    #         #dzcdt - (zcref - zc) / tp[1]
    #         ]
    dxdt = [(pbc/tc)*dpbhdt-b1 / V1 * (qr - q),
            (pwc/tc)*dpwhdt -b2 / V2 * (q - qch),
            (qc/tc)*dqdt - 1 / M * (pbh - pwh - rho * g * hw - F1 - F2 + rho * g * H)
            ]
    print(dxdt)
    return dxdt
...
print(pr)
print(pbc)
xssn = xss*np.array([1/pbc,1/pwc,1/qc])
print(xss)
