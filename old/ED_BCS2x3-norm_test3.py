...
data = np.load('BCS_data_train_norm.npz')
x1=data['x1']/xc[0]
x2=data['x2']/xc[1]
x3=data['x3']/xc[2]
tempo=data['t']/tc
...
variable = dde.callbacks.VariableValue(dx, period=1, filename=fnamevar)

model = dde.Model(data, net)

checker = dde.callbacks.ModelCheckpoint(
        "model/modelo2.ckpt", save_better_only=True)
...
modelo1="model/modelo2.ckpt-"+str(passos)
yhat=previsao(modelo1,[x1,x2,x3],tempo)
f = model.predict(tempo, operator=ED_BCS)
print("L2 relative error:", dde.metrics.l2_relative_error([x1,x2,x3], yhat))
